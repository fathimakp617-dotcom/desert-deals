...
const mapDbToProduct = (db: DbProduct): Product => {
  ...
  return {
    ...
    price: db.price,
    originalPrice: db.name.toLowerCase().includes("mind") ? Math.ceil(db.price / 0.9) : (db.original_price || db.price * 2),
    discountPercent: db.name.toLowerCase().includes("mind") ? 10 : (db.discount_percent || 0),
