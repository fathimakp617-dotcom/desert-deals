...
const Footer = memo(() => {
  ...
  const quickLinks = [
    { label: t("footer.shippingInfo"), to: "/shipping-policy" },
    { label: t("footer.returnsRefunds"), to: "/cancellation-refund-policy" },
    { label: t("footer.trackOrder"), to: "/account" },
    { label: t("footer.helpFaq"), to: "/contact" },
  ];

  const aboutLinks = [
    { label: t("footer.aboutUs"), to: "/#about" },
    { label: t("footer.ourBrands"), to: "/shop" },
    { label: t("footer.contactUs"), to: "/contact" },
  ];

  const bottomPolicyLinks = [
    { label: t("footer.privacyPolicy"), to: "/privacy" },
    { label: t("footer.shippingPolicy"), to: "/shipping-policy" },
    { label: t("footer.terms"), to: "/terms" },
    { label: t("footer.returnsPolicy"), to: "/cancellation-refund-policy" },
  ];
    ...
    <footer className="bg-black text-white overflow-x-hidden">
      <div className="container mx-auto px-6 lg:px-12 pt-14 pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="space-y-5 flex flex-col items-center sm:items-start">
            ...
            <p className="text-sm text-white/50 leading-relaxed max-w-xs text-center sm:text-start">
              {t("footer.tagline")}
            </p>
            <div className="flex items-center gap-4 pt-2">
              <a href="https://www.instagram.com/desertdeal.ae/" target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-white transition-colors" aria-label="Instagram">
                <Instagram className="w-5 h-5" />
              </a>
              <a href="https://wa.me/971501234567" target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-white transition-colors" aria-label="WhatsApp">
          ...
          <div className="space-y-4">
            ...
            <div className="relative mt-2 w-full max-w-full">
              <input
                ...
                className="w-full bg-white rounded-full px-5 py-3 text-sm text-black placeholder:text-black/40 focus:outline-none focus:ring-2 focus:ring-white/30 transition-colors pe-12"
              />
              <button className="absolute end-1 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center hover:bg-black/5 transition-colors" aria-label="Subscribe">
      ...
      <div className="container mx-auto px-6 lg:px-12 pb-8">
        <div className="flex flex-col items-center gap-4">
          <p className="text-xs text-white/40 font-medium uppercase tracking-widest">{t("footer.weAccept")}</p>
      ...
      <div className="border-t border-white/10">
        <div className="container mx-auto px-6 lg:px-12 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/40">
            © {new Date().getFullYear()} Desert Deal. {t("footer.rights")}
          </p>
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {bottomPolicyLinks.map((link) => (
              <Link key={link.to} to={link.to} className="text-xs text-white/40 hover:text-white transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
