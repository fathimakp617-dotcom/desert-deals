    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          ...
          text: `Order ${order.order_number} - Desert Deal`,
        });
      } else {
        ...
        setTimeout(() => {
          window.open(`https://wa.me/${phone}?text=${encodeURIComponent(`Order ${order.order_number} - Please find the order details in the image shared.`)}`, '_blank');
        }, 500);
      }
    } catch (err) {
